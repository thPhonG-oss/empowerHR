...
export default function MonthlyPointPage() {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      if (editing.type === "position") {
        await monthlyRewardApi.updateMonthlyReward(editing.id, {
          monthlyPoints: numValue,
        });
        setPositionData((prev) =>
          prev.map((p) => (p.id === editing.id ? { ...p, point: numValue } : p))
        );
        toast.success("Cập nhật điểm chức vụ thành công!");
      }

      if (editing.type === "department") {
        await monthlyRewardApi.updateDepartmentPoint(editing.id, {
          budget: numValue,
        });
        setDepartmentData((prev) =>
          prev.map((d) => (d.id === editing.id ? { ...d, point: numValue } : d))
        );
  ...
  const renderActions = (item, type) => {
    ...
    return isEdit ? (
      <div className="flex gap-2">
        <button
          onClick={saveEdit}
          className="cursor-pointer p-2 hover:bg-green-50 rounded-lg transition-colors group"
          title="Lưu"
        >
        ...
        <button
          onClick={cancelEdit}
          className="cursor-pointer p-2 hover:bg-red-50 rounded-lg transition-colors group"
          title="Hủy"
        >
  ...
  const totalPositionPoints = positionData.reduce(
    (sum, p) => sum + Number(p.point),
    0
  );
  const totalDepartmentBudget = departmentData.reduce(
    (sum, d) => sum + Number(d.point),
    0
  );
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 border border-gray-200">
          ...
          <p className="text-gray-600 ml-14">
            Thiết lập và quản lý điểm thưởng theo chức vụ và phòng ban
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium mb-1">
                  Tổng ngân sách chức vụ
                </p>
                <p className="text-3xl font-bold text-gray-900">
                  {totalPositionPoints.toLocaleString()}
                </p>
                <p className="text-gray-500 text-sm mt-2">
                  {positionData.length} chức vụ
                </p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-gray-600 text-sm font-medium mb-1">
                  Tổng ngân sách phòng ban
                </p>
                <p className="text-3xl font-bold text-gray-900">
                  {totalDepartmentBudget.toLocaleString()}
                </p>
                <p className="text-gray-500 text-sm mt-2">
                  {departmentData.length} phòng ban
                </p>
              </div>
              <div className="p-4 bg-blue-50 rounded-lg">
          ...
          <Table
            ...
            renderRow={(item) => (
              <>
                ...
                <td className="px-6 py-4">{renderActions(item, "position")}</td>
              </>
            )}
          />
        </Section>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-800">
                Giá trị phải là số nguyên dương và không vượt quá 1,000,000.
